...
const parse_cookie_map = (cookie_header = "") => {
  ...
  for (const cookie_pair of String(cookie_header).split(";")) {
    ...
    const raw_value = raw_value_parts.join("=").trim();
    try {
      cookie_map[cookie_key] = decodeURIComponent(raw_value);
    } catch (error) {
      if (!(error instanceof URIError)) throw error;
      // Keep a malformed preference present so it cannot revive a legacy value.
      cookie_map[cookie_key] = raw_value;
    }
  }
